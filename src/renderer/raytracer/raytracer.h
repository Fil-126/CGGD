...
#pragma omp parallel for
				for (int y = 0; y < height; y++)
				{
					...
					auto& history_pixel = histori->item(x, y);
	...
	{
		float3 int_ray_direction = float3(1.f) / ray.direction;
		float3 t0 = (aabb_max - ray.position) * int_ray_direction;
		float3 t1 = (aabb_min - ray.position) * int_ray_direction;
